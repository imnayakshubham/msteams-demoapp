...
server.post("/api/create-conversion", async (req, res) => {
  try {
    ...
    const conversationParameters = {
      ...
      members: [{ id: payload.id, name: payload.name } // Provide the user ID as the recipient
      ],
      isGroup: false,
      activity: null,
      channelData: null
    ...
    res.send(200, { conversationId: conversationResourceResponse.id });
  } catch (error) {
    console.error(error);
    res.send(500, { error: 'Failed to create conversation' });
  }
